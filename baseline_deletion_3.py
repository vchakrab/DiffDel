...
def ilp_approach_matching_java(cursor, table, key, target_attr, target_time, hypergraph, boundaries):
    ...
    while cells_to_visit:
        ...
        if edges:
            for edge in edges:
                ...
                # Constraint: sum(tji) >= bi (if RDR addressed, delete at least one tail)
                if tji_vars:
                    model.addConstr(quicksum(tji_vars) >= bi,
    ...
    return to_delete, solve_time, len(cell_to_id)


def delete_ilp_java_style(target: str, key: int, dataset: str, threshold: float):
    ...
    try:
        ...
        to_del, ilp_time, num_cells = ilp_approach_matching_java(
...
if __name__ == "__main__":
    ...
    cells, t, mem, depth, num = delete_ilp_java_style(target, key, dataset, threshold)

    print(f"{'='*70}")
    print(f"FINAL RESULTS:")
    print(f"  Cells deleted: {cells}")
    print(f"  Time: {t:.4f}s")
    print(f"  Cells instantiated: {num}")
    print(f"{'='*70}")
