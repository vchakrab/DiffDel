...
CATEGORIES = [
    ...
    ("Surprisal",     "surprisal",          "linear"),  # linear
...
def parse_standardized_csv(file_path: str):
    """
    Reads standardized CSV rows and returns data for star plots.
    """
    data = {}
    ...
    with open(file_path, "r", newline="") as f:
        ...
        for row in reader:
            ...
            data[ds].append({
                ...
                "space_overhead": float(max(0.0, memory_mb)),
...
def calculate_aggregated_metrics(dataset_data):
    ...
    # Calculate surprisal: -log2(leakage), with a small epsilon for leakage=0
    surprisal = -np.log2(1 - leakage_avg + 1e-9) if leakage_avg > 0 else -np.log2(1 - 1e-9)
    surprisal = max(0.0, surprisal) # Ensure surprisal is not negative
...
def create_star_plot(ax, metrics_data, dataset_name, axis_max):
    ...
    for b in baseline_order:
        ...
        for val, (title, key, scale) in zip(raw, CATEGORIES):
            vmax = axis_max[title]
            if scale == "log":
                norm.append(log_norm(val, vmax))
            else:
                norm.append(lin_norm(val, vmax))
...
def compute_axis_max_for_dataset(metrics_data):
    ...
    for (title, key, scale) in CATEGORIES:
        vals = [float(m.get(key, 0.0)) for _, m in metrics_data.items() if m]
...
if __name__ == "__main__":
    ...
    out = "star_plots_AllBaselines_v13_surprisal.pdf"
