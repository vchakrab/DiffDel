    ...
    for dataset, attr in zip(datasets, attributes):
        ...
        for i in range(100):
            try:
                ...
                mask_size = len(mask)
                leakage = calculate_leakage_str(hyperedges, all_paths, mask, attr,
                                                initial_known_for_path_finding, edge_weights)
                active_paths = _count_active_paths(hyperedges, all_paths, mask,
                                                   initial_known_for_path_finding)
...
def collect_exponential_deletion_data(epsilon, verbose: bool = False):
    """
    ...
    for dataset, attr in zip(datasets, attributes):
        ...
        header_verbose = [
            ...
            "leakage", "utility", "mask_size", "num_paths",
            "memory_overhead_bytes", "num_instantiated_cells"
        ]

        header_simple = ["epsilon", "leakage", "utility", "mask_size"]

        # write header ONCE
        with open(data_file_name, mode = "a", newline = "") as csv_file:
            writer = csv.writer(csv_file)
            if verbose:
                writer.writerow(header_verbose)
            else:
        ...
        for i in range(5):
            try:
                ...
                if results:
                    ...
                    with open(data_file_name, mode = 'a', newline = '') as csv_file:
                        if verbose:
                            ...
                            csv_row = (
                                f"{epsilon},{results['leakage']},{results['utility']},"
                                f"{results['mask_size']}\n"
                            )
                            csv_file.write(csv_row)
                ...
                print(
                    f"Error in exponential deletion experiment, dataset {dataset}, iteration {i + 1}: {e}")
                continue
...
def collect_2phase_deletion_data():
    ...
    with open(data_file_name, mode = 'w', newline = '') as csv_file:
        pass  # Clear file
        ...
        for i in range(100):
            try:
                ...
                results = two_phase_deletion.two_phase_deletion_main(
                    dataset = dataset, key = chosen_row, target_cell = attr,
                    templates = {attr: all_templates[attr]}
                )
                total_time = time.time() - start_time
                ...
                if results:
                    ...
                    with open(data_file_name, mode="a", newline="") as f:
                        ...
                        if verbose:
                            writer.writerow([
                                attr, total_time,
                                results["init_time"], results["model_time"], results["del_time"],
                                results["leakage"], results["utility"],
                                results["mask_size"], results["num_paths"],
                                results["memory_overhead_bytes"], results["num_instantiated_cells"]
                            ])
                        else:
                            writer.writerow([
