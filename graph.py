...
#FIGURE 19
def plot_heatmap_5(df: pd.DataFrame) -> plt.Figure:
    df = df.copy()
    df["improvement"] = 100 * (df["min_mask"] - df["mask_size"]) / df["min_mask"]
    eps_vals = sorted(df["epsilon_m"].unique())
    L0_vals  = sorted(df["L0"].unique())
...
#FIGURE 8 - Sensitivity of Exp and Gum to dependency leakage threshold and mask-pattern privacy budget
def plot_combined_3_with_panels(df: pd.DataFrame) -> plt.Figure:
    ...
    PANEL_PAD_Y = 0.038       # vertical margin — extra room above title and below x-ticks

    ordered = [(ds, "exp") for ds in DATASETS_3] + [(ds, "gum") for ds in DATASETS_3]

    fig, axes = plt.subplots(
        2, 6,
        figsize=(19.8, 6.5),   # same natural size as the two single-row originals stacked
        sharey="row",
        sharex=False,
    )
...
#FIGURE 14
def plot_pareto(df: pd.DataFrame) -> plt.Figure:
    """1-row × 5-col Pareto figure with cabinet-panel shading.

    Each panel shows the leakage–mask-size trade-off for one dataset,
    with two ε_m values and two mechanisms (Exp / Gum).

    Panel colours, font sizes, legend placement, and shading constants
    all match plot_mask_leakage_split_with_panels.
    """
    EM_VALS   = [0.1, 1.0]
    EPS_COLOR = {1.0: "#d62728", 0.1: "#1f77b4"}
    ...
    # ── Per-dataset panel colours (same LaTeX palette) ────────────────────────
    PANEL_COLORS = {
        "airport":  "#E1EBF8",
        "hospital": "#E4F3E4",
        "flight":   "#EBE1F5",
    ...
    PANEL_ALPHA       = 0.75
    PANEL_PAD_X       = 0.005
    PANEL_PAD_Y       = 0.072   # slightly more generous: single row with legend above
        ...
        for mech in ["Exp", "Gum"]:
            for em in EM_VALS:
                ...
                ax.plot(
                    ...
                    markersize=6,
                    linewidth=1.8,
                    label=rf"{mech_label}, $\varepsilon_m={em}$",
                )
...
#FIGURE 10, 15
def build_runtime() -> plt.Figure:
    """1-row × 5-col runtime decomposition figure with cabinet-panel shading.
    ...
    for i, dataset in enumerate(DATASET_ORDER):
        ...
        if ylim_max is not None:
            ...
            ax.set_yticks(yticks)
            if yticklabels is not None:          # [] → ticks present, no numbers
...
#FIGURE 20
def plot_budget_split_5(df: pd.DataFrame) -> plt.Figure:
    df = df.copy()
    df["improvement"] = 100 * (df["min_mask"] - df["mask_size"]) / df["min_mask"]
    agg = (df.groupby(["dataset", "mechanism", "epsilon_m", "L0"])["improvement"]
             .mean().reset_index())
...
#FIGURE 18 (a)
def plot_mask_split(df: pd.DataFrame, mech: str, legend: bool = True) -> plt.Figure:
...
def plot_leakage_split(df: pd.DataFrame, mech: str):
    ...
    fig, axes = plt.subplots(1, 5, figsize=(16.5, 3.5), sharey=False)
    plt.subplots_adjust(**{**SPLIT_ADJUST, "wspace": 0.15})
    subset_all = df[df["method"] == mech]
    for col, ds in enumerate(DATASETS_5):
        ax     = axes[col]
        subset = subset_all[subset_all["dataset"] == ds]
        ...
        if col != 0:
            ax.tick_params(axis="y", labelleft=False)
    fig.supxlabel(r"Dependency Leakage Threshold $L_0$", fontsize=SPLIT_SUP_FS + 4, y=-0.02)
    return fig

#FIGURE 12
def plot_mask_leakage_split(df: pd.DataFrame, mech: str, legend: bool = True) -> plt.Figure:
    """Combined 2-row × 5-col figure with cabinet-panel shading.

    Row 0: Mask-size improvement.
    Row 1: Achieved re-inference leakage.

    A single light-coloured rectangle spans both rows for each of the five
    columns, creating the "cabinet panel" grouping effect.  Dataset-specific
    colours follow the LaTeX palette:
        airport  → #E1EBF8  (rowA)
        hospital → #E4F3E4  (rowB)
        flight   → #EBE1F5  (rowD)
        adult    → #F8EAD2  (rowC  — light orange)
        tax      → #F5E1E1  (rowE  — light pink)

    Parameters
    ----------
    df : pd.DataFrame
        Must contain columns: dataset, method, epsilon_m, L0,
        improvement, ci_improvement, mean_leakage, ci_leakage.
    mech : str
        Mechanism to filter on (e.g. "exp" or "gum").
    legend : bool
        Whether to draw the shared legend above the figure.
    """
    # ── Per-dataset panel colours ─────────────────────────────────────────────
    PANEL_COLORS = {
        "airport":  "#E1EBF8",   # rowA: 225,235,248 — light blue
        "hospital": "#E4F3E4",   # rowB: 228,243,228 — light green
        "flight":   "#EBE1F5",   # rowD: 235,225,245 — light purple
        "adult":    "#F8EAD2",   # rowC: 248,234,210 — light orange
        "tax":      "#F5E1E1",   # rowE: 245,225,225 — light pink
    }
    PANEL_ALPHA       = 0.75    # translucency
    PANEL_PAD_X       = 0.006   # horizontal margin between columns
    PANEL_PAD_Y       = 0.038   # vertical margin above/below axes
    PANEL_PAD_X_LEFT0 = 0.021   # extra left pad for col 0 (covers y-label)
    ...
    # ── Cabinet-panel shading ─────────────────────────────────────────────────
    # Force layout so axis .get_position() returns final coordinates.
    fig.canvas.draw()
    ...
    for col, ds in enumerate(DATASETS_5):
        ...
        rect = plt.matplotlib.patches.FancyBboxPatch(
            ...
            alpha=PANEL_ALPHA,
            transform=fig.transFigure,
            zorder=0,       # behind all axes content
            clip_on=False,
        )
        fig.add_artist(rect)
    ...
    return fig
#FIGURE 17 (a)
def plot_leakage_split_gum(df: pd.DataFrame, mech: str) -> plt.Figure:
    """Leakage-split with legend and cabinet panels — for the Gum mechanism appendix."""
...
def plot_leakage_split_ablation_single(
    ...
    No legend (legend lives only on the gum figure stacked above it).
    Style, panel constants, and layout match plot_leakage_split_gum.
    """
    PANEL_COLORS = {
        "airport":  "#E1EBF8",
        "hospital": "#E4F3E4",
...
def graph_all_experiments():
    ...
    if not df_heat.empty:
        ...
        fig.savefig(FIG_DIR / "fig14_pareto_frontier_5.pdf", bbox_inches="tight"); plt.close(fig)

    if not df_main.empty:
        fig = build_runtime()
        fig.savefig(FIG_DIR / "fig10_runtime_breakdown_5.pdf", bbox_inches="tight"); plt.close(fig)

    if not df_heat.empty:
        fig = plot_heatmap_5(df_heat)
        fig.savefig(FIG_DIR / "fig19_heatmap_5.pdf", bbox_inches="tight"); plt.close(fig)
        fig = plot_budget_split_5(df_heat)
        fig.savefig(FIG_DIR / "fig20_budget_split_5.pdf", bbox_inches="tight"); plt.close(fig)

    fig = plot_mask_split(df_curves_5, "gum")
    fig.savefig(FIG_DIR / f"fig18_mask_gum_5.pdf", bbox_inches = "tight");
    plt.close(fig)
...
fig.savefig(FIG_DIR / "fig8_sensitivity_3.pdf", bbox_inches="tight")
plt.close(fig)
...
for variant in ["random", "zero"]:
    ...
    fig.savefig(FIG_DIR / f"fig18_mask_{variant}_5.pdf", bbox_inches="tight")
    plt.close(fig)

    fig = plot_leakage_split_ablation_single(df_abl, variant)
    fig.savefig(FIG_DIR / f"fig17_leakage_{variant}_5.pdf", bbox_inches="tight")
    plt.close(fig)
#
fig = plot_leakage_split_gum(df_curves_5, "gum")
fig.savefig(FIG_DIR / "fig17_leakage_gum_5.pdf", bbox_inches="tight")
plt.close(fig)


fig = plot_mask_vs_leakage_all_methods("data/release_data/all_masks_all_methods.csv")
fig.savefig(FIG_DIR / "fig16_leakage_models_5.pdf", bbox_inches="tight")
plt.close(fig)

graph_all_experiments()

fig = plot_heatmap_3_with_panels(df_heat)
fig.savefig(FIG_DIR / "fig11_heatmap_3.pdf", bbox_inches="tight")
plt.close(fig)

fig = plot_pareto_3_datasets(df_heat)
fig.savefig(FIG_DIR / "fig9_pareto_frontier_3.pdf", bbox_inches="tight")
plt.close(fig)

for mech in ["exp", "gum"]:
    fig = plot_mask_leakage_split(df_curves_5, mech)
    if mech != "gum":
        fig.savefig(FIG_DIR / f"fig12_sensitivity_{mech}_5.pdf", bbox_inches="tight")
    else:
        fig.savefig(FIG_DIR / f"fig13_sensitivity_{mech}_5.pdf", bbox_inches = "tight")
    plt.close(fig)