...
Same plots as graph.py but expanded to all 5 datasets in a 2-row layout
(row 0 = Exp, row 1 = Gum):

  fig_heatmap_5.pdf         — 2×5 heatmap  (was 1×6 for 3 datasets)
  fig_budget_split_5.pdf    — 2×5 best/worst mask improvement at iso-tau
  fig_runtime_5.pdf         — 2×5 stacked runtime bars + deletion ratio
  fig_mask_leakage_5.pdf    — 2×5 mask improvement curves (top) and
                               leakage curves (bottom) — 4 rows total

Data layout (same as graph.py):
    <script_dir>/data/exp/<dataset>/full_data.csv
    <script_dir>/data/gum/<dataset>/full_data.csv
    <script_dir>/data/min/<dataset>/full_data.csv   (runtime only)
"""
...
# =============================================================================
# SHARED CONFIG  (identical to graph.py)
# =============================================================================
...
METHOD_COLORS = {"DelMin":"#5B8FF9", "DelExp":"#5AD8A6", "DelMarg":"#F6BD16"}
...
# =============================================================================
# DATA LOADERS  (identical logic to graph.py)
# =============================================================================
def load_heatmap_data() -> pd.DataFrame:
    ...
    frames = [load_folder(DATA_DIR / "exp", "Exp"), load_folder(DATA_DIR / "gum", "Gum")]
...
# =============================================================================
# FIG 1 — HEATMAP  2×5
# (same cell style as graph.py's 1×6 but for all 5 datasets, rows=mechs)
# =============================================================================
def plot_heatmap_5(df: pd.DataFrame):
    ...
    FS_HM = 11  # smaller font for heatmap ticks/labels
    ...
    for row, mech in enumerate(["Exp","Gum"]):
        for col, ds in enumerate(DATASETS_5):
            ...
            im_ref = ax.imshow(pivot.values, cmap=PASTEL_CMAP, norm=norm,
            ...
            ax.xaxis.label.set_size(FS_HM)
            # ax.yaxis.label.set_size(FS_HM)
...
# =============================================================================
# FIG 2 — BUDGET SPLIT  2×5
# (same bar style as graph.py's generate_budget_table but as a figure)
# =============================================================================
def plot_budget_split_5(df: pd.DataFrame):
    ...
    for row, mech in enumerate(["Exp","Gum"]):
        for col, ds in enumerate(DATASETS_5):
            ...
            if row == 0:
                ax.set_title(rf"$\mathbf{{{ds.capitalize()}}}$", pad=6, fontsize=FS)
            print(ds, mech, valid_taus, best_vals, worst_vals)
...
# =============================================================================
# FIG 3 — RUNTIME  2×5
# (same stacked-bar style as graph.py's build_runtime, 2 rows for Exp/Gum)
# =============================================================================
def plot_runtime_5(df_main: pd.DataFrame):
    """
    Single row showing all 3 methods (DelMin / DelExp / DelMarg).
    Each panel: stacked runtime bars (left axis) + deletion ratio bars (right twin).
    """
    ALL_METHODS = ["DelMin", "DelExp", "DelMarg"]
    ...
    for col, dataset in enumerate(DATASETS_5):

        ax = axes[col]

        ddf = df_main[
            (df_main["dataset"] == dataset.capitalize()) &
            (df_main["method"].isin(ALL_METHODS))
        ]
        ...
        ax.set_xticks(x - w)
        ax.set_xticklabels([METHOD_LABEL[m] for m in ALL_METHODS], fontsize=FS)
        ...
        ax2.bar(
            x + w, instantiated_pct, width=w,
            color=ZONE_COLOR_LIGHT,
            edgecolor="black",
            linewidth=0.3
        )
        ax2.bar(
            x + w, deleted_pct, width=w,
        ...
        # -------------------------
        if col == 0:
            ax.set_ylabel("Time (ms)", fontsize=FS)
        else:
            ax.set_ylabel(None)

        if col == len(DATASETS_5) - 1:
            ax2.set_ylabel("Deleted Cells (%)", fontsize=FS)
        else:
            ax2.set_ylabel(None)
    ...
    return fig
# =============================================================================
# FIG 4 — MASK IMPROVEMENT  2×5
# (same line style as graph.py's plot_mask_curves, rows=mechs)
# y-axis label on every subplot; legend shown
# =============================================================================
def plot_mask_curves_5(df: pd.DataFrame):
    fig, axes = plt.subplots(2, 5, figsize=(16.5, 6.5), sharey=False)
    plt.subplots_adjust(hspace=0.35, wspace=0.18,
                        left=0.07, right=0.97, top=0.88, bottom=0.10)

    for row, mech in enumerate(["exp","gum"]):
        mech_label = "Gum" if mech == "gum" else "Exp"
        ...
        for col, ds in enumerate(DATASETS_5):
            ...
            if row == 0:
                ax.set_title(rf"$\mathbf{{{ds.capitalize()}}}$", pad=2, fontsize=FS)
            ax.set_xticks([0, 0.2, 0.4, 0.6, 0.8, 1])
            ax.set_xlim(0.0, 0.9)
            ax.set_ylim(0, 100)
            ax.set_yticks([0, 25, 50, 75, 100])
            ax.set_ylabel("Mask Size Improvement (%)", fontsize=FS)
    ...
    fig.supxlabel(r"Re-inference Threshold $L_0$", fontsize=FS, y=0.01)

    return fig


# =============================================================================
# FIG 5 — LEAKAGE CURVES  2×5
# (same line style as graph.py's plot_leakage_curves, rows=mechs)
# y-axis label on every subplot; no legend
# =============================================================================
def plot_leakage_curves_5(df: pd.DataFrame):
    fig, axes = plt.subplots(2, 5, figsize=(16.5, 6.5), sharey=False)
    ...
    for row, mech in enumerate(["exp","gum"]):
        ...
        for col, ds in enumerate(DATASETS_5):
            ...
            ax.set_ylabel("Achieved Re-inference Leakage (%)", fontsize=FS)
...
if __name__ == "__main__":
    ...
    df_curves = load_curves_data(DATASETS_5)
    df_main   = load_main_data()

    print(f"  heatmap rows : {len(df_heat):,}")
    print(f"  curves  rows : {len(df_curves):,}")
    print(f"  main    rows : {len(df_main):,}")

    print("\nWriting individual PDFs ...")

    if not df_heat.empty:
        print("  fig_heatmap.pdf ...")
        plot_heatmap_5(df_heat).savefig("fig_heatmap.pdf", bbox_inches="tight")
        plt.close()

        print("  fig_budget_split.pdf ...")
        plot_budget_split_5(df_heat).savefig("fig_budget_split.pdf", bbox_inches="tight")
        plt.close()

    if not df_curves.empty:
        print("  fig_method_mask_improvement.pdf ...")
        plot_mask_curves_5(df_curves).savefig("fig_method_mask_improvement.pdf", bbox_inches="tight")
        plt.close()

        print("  fig_method_leakage_percentage.pdf ...")
        plot_leakage_curves_5(df_curves).savefig("fig_method_leakage_percentage.pdf", bbox_inches="tight")
        plt.close()

    if not df_main.empty:
        print("  fig_runtime.pdf ...")
        plot_runtime_5(df_main).savefig("fig_runtime.pdf", bbox_inches="tight")
        plt.close()
