...
def exponential_deletion_main(dataset: str, key: int, target_cell: str):
    ...
    """
    print("\n" + "=" * 70)
    print(f"Running Main Orchestrator for Dataset: '{dataset}', Key: {key}, Target: '{target_cell}'")
    print("=" * 70)

    # --- Initialization Phase ---
    init_start = time.time()
    try:
        dc_module_path = f'DCandDelset.dc_configs.top{dataset.capitalize()}DCs_parsed'
...
def run_small_test_example():
    """
    Runs a small, self-contained example with user-defined constraints
    to demonstrate the exponential deletion mechanism.
    """
    print("\n" + "=" * 70)
    print("Running Small User-Defined Test Example")
    print("=" * 70)

    # 1. Define the user's constraints, which act as the hyperedges
    hyperedges = [
        ('a', 'b', 'c', 'd'),  # DC 0
        ('d', 'e', 'f')       # DC 1
    ]
    print(f"Hyperedges (Constraints): {hyperedges}")

    # 2. Define the target cell to be deleted
    target_cell = 'c'
    print(f"Target Cell: '{target_cell}'")

    # 3. Calculate the initial_known set based on the "intersecting cells" logic
    all_attributes = set(attr for he in hyperedges for attr in he)
    attribute_counts = Counter(attr for he in hyperedges for attr in he)
    intersecting_cells = {attr for attr, count in attribute_counts.items() if count > 1}
    initial_known = all_attributes - {target_cell} - intersecting_cells
    ...
    print(f"Intersecting Cells (in >1 DC): {intersecting_cells}")
    print(f"Calculated Initial Known Set: {initial_known}")
    
    # 4. Define weights and algorithm parameters
    edge_weights = {0: 0.9, 1: 0.8} # High confidence in both rules
    alpha = 1.0
    beta = 0.5  # Cost of deleting one attribute
    epsilon = 1.0
    
    # 5. Run the exponential deletion mechanism
    # Note: This part is now handled inside exponential_deletion_main,
    # so this test function is slightly out of sync but still illustrative.
    paths = find_inference_paths_str(hyperedges, target_cell, initial_known)
    inference_zone = get_path_inference_zone_str(paths, hyperedges, target_cell)
    candidates = compute_possible_mask_set_str(inference_zone)
    final_mask = exponential_mechanism_sample_str(
        candidates, target_cell, paths, hyperedges, initial_known,
        edge_weights, alpha, beta, epsilon
    )
    
    print("\n" + "=" * 70)
    print(f"Small Example Finished. Final deletion mask: {final_mask}")
    print("NOTE: The small example does not perform a database update or full metric calculation.")
    print("=" * 70)
    return final_mask


if __name__ == '__main__':
    # Run the main orchestrator with all parameters passed explicitly
