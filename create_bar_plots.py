...
def create_combined_bar_plot(all_metrics, datasets, baselines):
    ...
    for i, dataset in enumerate(datasets):
        ...
        ax1.bar(x - w/2, model + eps, w,
                bottom=init + eps,
                color=colors['Modeling Time'],
                label=r"Modeling Time")
        ax1.bar(x - w/2, delete + eps, w,
                bottom=init + model + eps,
                color=colors['Update to NULL Time'],
                label=r"Update to Null Time")

        ax2.bar(x + w/2, memory + eps, w,
                color=colors['Model Size'],
                label=r"Model Size")
...
def main():
    ...
    fig.savefig("bar_plot_AllBaselines_NoTax_v12.pdf", bbox_inches="tight")
    plt.close(fig)

    print("Saved bar_plot_AllBaselines_NoTax_v12.pdf")
